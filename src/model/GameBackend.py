...
class GameBackend:
    ...
    def add_couple(self, couple_id, name) -> None:
        self.queue_couples.append({'id': couple_id, 'arrival': date.get_current_time()})
        self.player_names[couple_id] = name

    def add_single(self, single_id, name) -> None:
        self.queue_singles.append({'id': single_id, 'arrival': date.get_current_time()})
        self.player_names[single_id] = name

    def add_couple2(self, couple_id, name) -> None:
        self.queue_couples2.append({'id': couple_id, 'arrival': date.get_current_time()})
        self.player_names[couple_id] = name

    def add_single2(self, single_id, name) -> None:
        self.queue_singles2.append({'id': single_id, 'arrival': date.get_current_time()})
        self.player_names[single_id] = name
    ...
    def record_couple_game(self, timer_duration: float, official_score: float) -> None:
        ...
        self.couple_history_total.append(
            (self.current_player_couple['id'], official_score))  # Store player_id with score
    ...
    def record_single_game(self, timer_duration: float, official_score: float) -> None:
        ...
        if self.current_player_alfa and self.current_player_alfa.get('id', '').startswith(
                "BLU"):  # Controllo aggiunto per sicurezza
            player_id_stopped = self.current_player_alfa['id']
            current_player_copy = self.current_player_alfa.copy()  # Copia prima di cancellare
            logging.info(
                f"[Record Single] Start processing for {player_id_stopped}. Timer={timer_duration:.4f}, Score={official_score:.4f}")
            ...
            self.single_in_alfa = False
            logging.debug(
                f"[Record Single] current_player_alfa is now {self.current_player_alfa}, single_in_alfa is {self.single_in_alfa}")
            ...
            logging.warning(
                f"[Record Single] Called but no active Single player (BLU) found in ALFA. Current: {current_id}")
    ...
    def check_qualification(self, score_minutes: float, player_type: str) -> Tuple[bool, Optional[str]]:
        """
        Controlla se un punteggio si qualifica confrontandolo con i top 3
        attualmente presenti nella tabella 'qualified_players'.
        ...
        logging.debug(
            f"[QUAL CHECK NEW] Checking score {score_minutes} for type {player_type} against qualified players.")
        ...
        try:
            ...
            if len(top_qualified_scores) < 3:
                ...
                logging.debug(
                    f"[QUAL CHECK NEW] Qualifies because less than 3 players are currently qualified for type {player_type}.")
            # elif score_minutes <= top_qualified_scores[-1] + 1e-9: # Aggiungi tolleranza se lavori con float che possono avere imprecisioni
                ...
                if score_minutes <= top_player_today[0]:
                    ...
                    logging.debug(
                        f"[QUAL CHECK NEW] Qualifies by beating or matching the current 3rd best qualified score ({top_qualified_scores[-1]}) for type {player_type}.")
                ...
                logging.debug(
                    f"[QUAL CHECK NEW] Does not qualify. Score {score_minutes} is not better than the 3rd best qualified score ({top_qualified_scores[-1]}) for type {player_type}.")

        except sqlite3.Error as e:
            # Gestione specifica per errori SQLite (es. tabella non trovata, colonna mancante)
            logging.error(
                f"[QUAL CHECK NEW] Database error checking qualification for {player_type} score {score_minutes}: {e}",
                exc_info=True)
            ...
            if "no such table" in str(e).lower():
                ...
                logging.error(
                    f"[QUAL CHECK NEW] **** ERROR: Check if 'qualified_players' table has columns named 'score' and 'player_type'. ****")
            return False, None  # Errore durante il check
        except Exception as e:
            # Gestione per altri errori generici
            logging.error(
                f"[QUAL CHECK NEW] General error checking qualification for {player_type} score {score_minutes}: {e}",
                exc_info=True)
    ...
    def record_charlie_game(self, timer_duration: float) -> None:
        ...
        if self.current_player_charlie:
            ...
            logging.info(
                f"[CHARLIE TIMER RECORD] Player {player_id_stopped} stopped. Timer duration: {timer_duration:.4f} minutes.")
    ...
    def update_averages(self) -> None:
        ...
        if len(self.couple_timer_history) >= min_games_for_avg:
            ...
            self.T_total = self.default_T_total
            logging.debug(
                f"Using default T_total: {self.T_total:.4f} (timer records: {len(self.couple_timer_history)})")
        ...
        if len(self.single_timer_history) >= min_games_for_avg:
            ...
            logging.debug(
                f"Calculated T_single from {len(self.single_timer_history)} TIMER records: {self.T_single:.4f}")
        else:
            self.T_single = self.default_T_single
            logging.debug(
                f"Using default T_single: {self.T_single:.4f} (timer records: {len(self.single_timer_history)})")
        ...
        if len(self.single_timer_history2) >= min_games_for_avg:
            ...
            logging.debug(
                f"Calculated T_single from {len(self.single_timer_history2)} TIMER records: {self.T_single2:.4f}")
        else:
            self.T_single2 = self.default_T_single
            logging.debug(
                f"Using default T_single: {self.T_single2:.4f} (timer records: {len(self.single_timer_history2)})")
        ...
        if len(self.charlie_timer_history) >= min_games_for_avg:
            ...
            logging.debug(
                f"Calculated T_charlie from {len(self.charlie_timer_history)} timer records: {self.T_charlie:.4f}")
        else:
            self.T_charlie = self.default_T_charlie
            logging.debug(
                f"Using default T_charlie: {self.T_charlie:.4f} (timer records: {len(self.charlie_timer_history)})")
        ...
        logging.info(
            f"Averages Updated: T_mid={self.T_mid:.2f}, T_total(timer)={self.T_total:.2f}, T_single(timer)={self.T_single:.2f}, T_mid2={self.T_mid2:.2f}, T_charlie={self.T_charlie:.2f}, T_statico={self.T_statico:.2f}")
    ...
    def update_next_player(self) -> None:
        ...
        """
        logging.debug(
            f"[UpdateNextPlayer1] Start Check. Alfa: {self.current_player_alfa is not None}, Bravo: {self.current_player_bravo is not None}")
    ...
    def update_next_player2(self) -> None:
        ...
        logging.debug(
            f"[UpdateNextPlayer2] Start Check. Alfa2 Occupied: {self.current_player_alfa2 is not None}, Bravo2 Occupied: {self.current_player_bravo2 is not None}")
        next_id = None
        next_name = None
        locked = False
        ...
        # Caso 2: ALFA2 libera, BRAVO2 occupato (Coppia ROSA sta finendo)
            ...
            logging.debug(
                "[UpdateNextPlayer2] Case: Alfa2 Free, Bravo2 Occupied. Priority to singles (ARANCIO/BIANCO).")
        ...
        # Caso 3: ALFA2 è occupata (non importa lo stato di BRAVO2)
            ...
            logging.debug(
                "[UpdateNextPlayer2] Case: Alfa2 Occupied (Bravo2 state irrelevant for *next* priority). Priority to couples (ROSA).")
            if self.queue_couples2:
                next_id = self.queue_couples2[0]['id']
                locked = True
        ...
        if is_couple:
            if self.queue_couples:
                ...
                if self.queue_singles:
                    ...
                    self.next_player_alfa_bravo_name = self.get_player_name(
                        self.next_player_alfa_bravo_id) if self.next_player_alfa_bravo_id else None
        ...
        if is_couple:
            if self.queue_couples2:
                ...
                if self.queue_singles2:
                    ...
                    self.next_player_alfa_bravo_name2 = self.get_player_name(
                        self.next_player_alfa_bravo_id2) if self.next_player_alfa_bravo_id2 else None
    ...
    def start_statico_game(self, pista: str) -> None:
        ...
        if pista == 'delta' and not self.current_player_delta:
            ...
            self.player_start_times[self.current_player_delta['id']] = date.get_current_time()
            ...
            self.player_start_times[self.current_player_echo['id']] = date.get_current_time()
            # Rimuovi il giocatore dalla coda
            self.queue_statico.pop(0)
    ...
    def get_waiting_board(self) -> Tuple[
                ...
                for queue_item in self.queue_couples + self.queue_singles:
                    ...
                    if estimated_time:
                        ...
                        if minutes_to_entry <= 2 or not next_player_found:
                            ...
                            self.next_player_alfa_bravo_name = self.get_player_name(
                                self.next_player_alfa_bravo_id)  # Aggiorna anche il nome
                            self.next_player_alfa_bravo_locked = True
        ...
        if not self.next_player_alfa_bravo_locked2:
            if self.couple_in_bravo2 and self.single_in_alfa2 and self.queue_couples2:
                self.next_player_alfa_bravo_id2 = self.queue_couples2[0]['id']
                self.next_player_alfa_bravo_name2 = self.get_player_name(
                    self.next_player_alfa_bravo_id2)  # Aggiorna anche il nome
                self.next_player_alfa_bravo_locked2 = True
            else:
    ...
    def button_third_pressed(self) -> None:
        ...
        if self.current_player_alfa and self.current_player_alfa.get("id", "").startswith("GIALLO"):
            ...
            if start_time:
                ...
                logging.debug(
                    f"[Button Third 1 {player_id}] Start time: {start_time.isoformat()}, Mid duration calculated: {mid_duration_minutes:.4f} min")
                ...
                logging.info(
                    f"[Button Third 1 {player_id}] Appended mid_duration {mid_duration_minutes:.4f} to couple_history_mid. Size: {list_size_before} -> {list_size_after}")
                ...
                logging.error(
                    f"[Button Third 1 {player_id}] CRITICAL: Start time not found! Cannot record mid_duration.")
            ...
            logging.debug(
                f"[Button Third 1 {player_id}] Resetting current_player_alfa (was {self.current_player_alfa}) and setting third_button_pressed=True")
            ...
            current_id = self.current_player_alfa.get("id", "None") if self.current_player_alfa else "None"
            logging.warning(
                f"[Button Third 1] Pressed but no Couple (GIALLO) found in ALFA. Current: {current_id}. Action ignored.")
    ...
    def button_third_pressed2(self) -> None:
        ...
        if self.current_player_alfa2 and self.current_player_alfa2.get("id", "").startswith("ROSA"):
            ...
            if start_time:
                ...
                logging.debug(
                    f"[Button Third 2 {player_id}] Start time: {start_time.isoformat()}, Mid2 duration calculated: {mid2_duration_minutes:.4f} min")
                ...
                logging.info(
                    f"[Button Third 2 {player_id}] Appended mid2_duration {mid2_duration_minutes:.4f} to couple_history_mid2. Size: {list_size_before} -> {list_size_after}")

                # self.update_averages() # Valuta se chiamarlo qui

            else:
                logging.error(
                    f"[Button Third 2 {player_id}] CRITICAL: Start time not found! Cannot record mid2_duration.")
            ...
            logging.debug(
                f"[Button Third 2 {player_id}] Resetting current_player_alfa2 (was {self.current_player_alfa2}) and setting third_button_pressed2=True")
            ...
            logging.warning(
                f"[Button Third 2] Pressed but no Couple2 (ROSA) found in ALFA2. Current: {current_id}. Action ignored.")
    ...
    def can_stop_couple(self) -> bool:
        ...
        can_stop = self.third_button_pressed and \
                   ...
                   self.current_player_couple.get("id", "") == self.current_player_bravo.get("id",
                                                                                             "")  # Assicurati che sia la stessa coppia
    ...
    def calculate_yellow_waiting_time(self, current_giallo: int, len_blu: int, dt_total: float, dt_mid: float,
                                      dt_single: float) -> float:
        """
