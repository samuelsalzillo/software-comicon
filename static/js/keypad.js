...
function checkCode() {
    if (keypadInput === currentCode) {
        if (currentPlayerAlfa && currentPlayerAlfa.id && currentPlayerAlfa.id.startsWith("GIALLO")) {
            pressThirdButton();
        } else {
...
function pressThirdButton() {
    ...
    .then((data) => {
        if (data.success) {
            ...
            showKeypad1Notification(`Errore: ${data.error || "Attivazione fallita."}`, true);
        }
    })
    .catch((error) => {
        console.error("Errore durante l'attivazione del pulsante third:", error);
        showKeypad1Notification(`Errore di comunicazione: ${error.message}`, true);
    });
}
...
function updateCurrentPlayerAlfa() {
    fetch("/simulate")
        ...
        .then((data) => {
            ...
            const displayElement = document.getElementById("current-player-alfa-display");
            if (displayElement) {
                displayElement.textContent = currentPlayerAlfa ? currentPlayerAlfa.id : "-";
            }
            ...
            if (currentPlayerAlfa && currentPlayerAlfa.id && currentPlayerAlfa.id.startsWith("GIALLO")) {
                ...
                if (!previousPlayer || !previousPlayer.id || !previousPlayer.id.startsWith("GIALLO")) {
...
function showKeypad1Notification(message, isError = false) {
    ...
    if (notificationElement) {
        ...
        console.warn("Elemento notifica #notification-keypad1 non trovato. Messaggio:", message);
    }
}
